...
class WarningIfMemberIncorrect {
    ...
    void m() {
        ...
        MethodHandle mh1 = ldc(MethodHandleDesc.of(MethodHandleDesc.Kind.STATIC, ClassDesc.ofDescriptor("LWarningIfMemberIncorrect;"), "foo", mtReturnString));

        // warn: referring to a field not to a method plus a warning because the symbol has the wrong staticness
        MethodHandle mh2 = ldc(MethodHandleDesc.of(MethodHandleDesc.Kind.STATIC, ClassDesc.ofDescriptor("LWarningIfMemberIncorrect;"), "bar", mtReturnString));

        // warn: wrong kind to refer to a constructor
        MethodHandle mhConstructor = ldc(MethodHandleDesc.of(MethodHandleDesc.Kind.VIRTUAL, ClassDesc.ofDescriptor("LWarningIfMemberIncorrect;"), "<init>",
                                                             MethodTypeDesc.of(ConstantDescs.CR_void)));

        MethodHandle mh3 = ldc(MethodHandleDesc.of(MethodHandleDesc.Kind.VIRTUAL, ClassDesc.ofDescriptor("LWarningIfMemberIncorrect$I;"), "foo", mtReturnString));
    }
