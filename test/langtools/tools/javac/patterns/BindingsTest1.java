/*
 * @test
 * @summary Basic tests for bindings from matches
...
public class BindingsTest1 {
    ...
    public static void main(String[] args) {
        ...
        if (o1 matches String s) {
            s.length();
        }

        // Test for e1 && e2.T = union(e1.T, e2.T)
        if (o1 matches String s && o2 matches Integer in) {
            s.length();
            in.intValue();
        }
        // Test for e1 && e2.F = intersect(e1.F, e2.F)
        if (!(o1 matches String s) && !(o1 matches String s)) {
        ...
        if (o1 matches String s && s.length()>0) {
            System.out.print("done");
        }

        // Test for (e1 || e2).T = intersect(e1.T, e2.T)
        if (o1 matches String s || o3 matches String s){
            System.out.println(s); // ?
        }

        // Test for (e1 || e2).F = union(e1.F, e2.F)
        if (!(o1 matches String s) || !(o3 matches Integer in)){
        } else {
            s.length();
            i.intValue();
        }
        ...
        if (Ktrue() ? o2 matches Integer x : o2 matches Integer x) {
            x.intValue();
        }

        // Test for (e1 ? e2 : e3).T contains intersect(e1.T, e3.T)
        if (o1 matches String s ? true : o1 matches String s) {
            s.length();
        }

        // Test for (e1 ? e2 : e3).T contains intersect(e1.F, e2.T)
        if (!(o1 matches String s) ? (o1 matches String s) : true) {
            s.length();
        }
